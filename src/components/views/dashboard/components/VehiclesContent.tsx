...
export default function VehiclesContent() {
  ...
  useEffect(() => {
    ...
    else if (selectedImageIndex >= editingImages.length)
      setSelectedImageIndex(0);
  }, [editingImages]);
  ...
  const handleDeleteImage = async (publicId: string) => {
    ...
    if (
      ...
      selectedImageIndex >= editingImages.length - 1
    )
  ...
  const handleAddImages = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      updated.images.forEach((img) => {
        if (!merged.some((i) => i.public_id === img.public_id))
          merged.push(img);
      });
    ...
    return (
      <div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 my-4">
          <p className="text-yellow-600">No tienes vehículos registrados</p>
        </div>
      </div>
    );
  }
            ...
            {editingVehicle === vehicle.id ? (
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <h3 className="font-semibold text-lg">Editar vehículo</h3>
                  <button
                  ...
                  {editingImages.length > 0 && (
                    <div className="flex flex-col md:flex-row gap-6">
                      <div className="w-full md:w-2/5 flex flex-col items-center">
                        <div
                          className="w-full rounded-lg bg-gray-100 flex items-center justify-center overflow-hidden border border-gray-200"
                        ...
                        <div className="mt-3 flex gap-2 overflow-x-auto scrollbar-thin w-full justify-center">
                          {editingImages.map((img, idx) => (
                            <div
                              key={img.public_id}
                              className={`relative border-2 rounded-md cursor-pointer transition-all duration-150 ${
                                selectedImageIndex === idx
                                  ? "border-principal-blue ring-2 ring-principal-blue"
                                  : "border-gray-200"
                              }`}
